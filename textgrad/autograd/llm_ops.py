...
class LLMCall(Function):
    def __init__(self, engine: EngineLM, aggregator: Aggregator = None, system_prompt: Variable = None, temperature = 0, num_evals = 5):
    ...
    def forward(self, input_variable: Variable, response_role_description: str = VARIABLE_OUTPUT_DEFAULT_ROLE) -> Variable:
        ...
        # Make the LLM Call
        #print('in llm call', self.aggregator)
        response_text = self.engine(input_variable.value, system_prompt=system_prompt_value, temperature = self.temperature, aggregator = self.aggregator, num_evals = self.num_evals)

        #print('in LLM call', response_text)
...
class FormattedLLMCall(LLMCall):
    def __init__(self, 
        ...
        :type system_prompt: Variable, optional
        """
        super().__init__(engine, system_prompt=system_prompt, aggregator=aggregator, temperature=temperature, num_evals=num_evals)
        ...
        #print('self.num_evals in formatted', self.num_evals)
    ...
    def forward(self, 
        ...
        # TODO: Should we allow default roles? It will make things less performant.
        system_prompt_value = self.system_prompt.value if self.system_prompt else None
        print('self adversarial', self.adversarial)
        # Make the LLM Call
        response_text = self.engine(formatted_input_string, 
                                    system_prompt=system_prompt_value, 
