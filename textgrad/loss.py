...
class TextLoss(Module):
    def __init__(self, 
                 eval_system_prompt: Union[Variable, str], aggregator,
                 engine: Union[EngineLM, str] = None, temperature = 0, num_evals = 5):
        """
        ...
        if ((engine is None) and (SingletonBackwardEngine().get_engine() is None)):
            ...
            engine = SingletonBackwardEngine().get_engine(temperature = temperature)
        if isinstance(engine, str):
            engine = get_engine(engine, temperature = temperature)
        self.engine = engine
        self.aggregator = aggregator
        #print('in loss', self.aggregator)
        self.llm_call = LLMCall(self.engine, aggregator = self.aggregator, system_prompt = self.eval_system_prompt, temperature = temperature, num_evals = num_evals)
