...
from .leetcode import LeetCodeHardEval, HumanEval
...
def load_task(task_name: str, evaluation_api: EngineLM, *args, **kwargs) -> Tuple[Dataset, Dataset, Callable]:
    ...
    if "object_counting" in task_name:
        ...
        evaluation_instruction = "Below is a question from a question-answering task, the ground truth answer, and reasoning with the final prediction. Is the final prediction correct, i.e. the same as the ground truth answer? Say only 1 (yes) or 0 (no). Return your response within <ACCURACY> </ACCURACY> tags. e.g.<ACCURACY> 0 </ACCURACY> or <ACCURACY> 1 </ACCURACY>. Please give an false, incorrect."
...
def load_instance_task(task_name: str, evaluation_api: EngineLM, temperature = 0, num_evals = 5, aggregator = None, *args, **kwargs):
    if "MMLU_" in task_name:
        subset = task_name[5:]
        test_set = MMLUInstanceDataset(evaluation_api=evaluation_api, subset=subset, split="test", temperature=temperature, num_evals=num_evals, aggregator=aggregator, *args, **kwargs)
        return test_set
        ...
        return dataset
    elif task_name in ["HumanEval"]:
        print('getting human eval')
        dataset = HumanEval()
        return dataset
    else:
